...
void maj_menu_clic(int x, int y,Menu *m)
{
        ...
        x> MAP_WIDTH*TILE_WIDTH - 10*TAILLE_CHAR_W*RENDER_SCALE && x<MAP_WIDTH*TILE_WIDTH - 2*TAILLE_CHAR_W*RENDER_SCALE)
        {
            m->lancer_jeu = 1;
        }

         if (y> 8*TILE_HEIGHT*RENDER_SCALE  && y < 8*TILE_HEIGHT*RENDER_SCALE +4*TAILLE_CHAR_H*RENDER_SCALE &&
        x> (MAP_WIDTH*TILE_WIDTH - 6*TAILLE_CHAR_W*RENDER_SCALE) && x<(MAP_WIDTH*TILE_WIDTH - 3*TAILLE_CHAR_W*RENDER_SCALE))
        {
            m->volume_son++;
            if (m->volume_son ==4)
                m->volume_son =0;
